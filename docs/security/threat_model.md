...
| 3 | **Compromised dependency / supply-chain attack** via a poisoned package or Action. | Dependabot for `pip` and `github-actions`; CodeQL for Python and JavaScript; GitHub Actions pinned to **major-version tags** (Dependabot tracks updates; SHA-pinning is a tracked hardening item — see §5); pre-commit secret scanning; production site ships **no third-party JS** (build/test dependencies do not reach the deployed bundle). | Major-version tags are mutable, so a re-pointed/compromised tag could be pulled before Dependabot/CodeQL flags it; SHA-pinning would close this. Transitive Python deps affect ingestion/CI, not the served site, but could still poison catalog data (see #6). |
