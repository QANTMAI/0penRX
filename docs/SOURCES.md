...
| **PPA / helpingpatients.org** (Partnership for Prescription Assistance) | Directory of manufacturer PAPs and eligibility programs (~475 programs) | Site Terms of Use must be captured and reviewed before any ingest; redistribution terms unconfirmed | HTML list, likely JS-rendered; no public JSON API, not CORS-callable. Needs a verified parser against the rendered DOM | **2 — Gated** |
| **NeedyMeds** | Comprehensive PAP, copay-card, and free/low-cost clinic data (~2,000 coupons · 375 PAPs) | Licensed data feed; **no free public API**. Requires a signed license to ingest or redistribute | No browser-callable API. Access is contractual, not technical | **2 — Gated** |
...
| **CoverMyMeds** (covermymeds.health) | Copay-program and prior-auth data | Partnership API; agreement required | No public API; server-to-server under contract | **3 — Gated** |
| **Team Cuban Card** (teamcubancard.com — Mark Cuban Cost Plus Benefits LLC) | Published retail cash prices (name · strength · form · quantity · price) for the generics carried on the card, honored at Cost Plus Drugs Affiliate Pharmacies. A **separate channel** from costplusdrugs.com mail order, with its own prices | Terms of Use (last updated Apr 14 2023) bar users from "replicat[ing]" the Site and from "use[ing] robots or scripts with the Site" — **no ingest, no mirroring of the price list, no automated fetch**. Membership is registration-gated; the site states no fee and the sign-up form takes no payment method, though it never uses the word "free" | Public list is JS-rendered (server HTML is a nav shell) with an official Excel export; per-member pricing sits behind a login at `costplusdrugs.myrxplan.com`. **Manual import of the official Excel export, by owner decision (2026-08-11)** — the owner was shown both ToS clauses and accepted the contractual risk of republishing. **No automated fetch of any kind**: `data/build_teamcuban.py` has zero network code and a test asserts it stays that way, because clause (v) bars robots/scripts on the Site. The raw export is gitignored — we publish the derived prices, not their file. Refresh is manual and the capture date ships to the UI | **3 — Gated (ToS forbids automated access; import is manual + owner-authorised)** |
