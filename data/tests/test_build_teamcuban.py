...
_OBSERVED = [
    ...
    ["Abacavir / Lamivudine\n(Generic For Epzicom)", "600-300 MG", "Tablet", "30 Tablets", "$ 42.34"],
    ["Acetylcysteine\n(Generic For Mucomyst)", "20 %", "Solution", "1 Carton (100 Ml)", "$ 410.99"],
...
def test_header_aliases_are_accepted(tmp_path):
    ...
    rows = build_teamcuban.load_rows(_write_csv(tmp_path, [
...
def test_xlsx_shared_strings_numeric_price_and_blank_column(tmp_path):
    ...
    """
    shared = ["Name", "Strength", "Form", "Quantity", "Price",
              "Abacavir / Lamivudine\n(Generic For Epzicom)", "600-300 MG", "Tablet",
              "30 Tablets", "Metformin Hcl", "500 MG", "1 Bottle (240 Ml)"]
    ns = "http://schemas.openxmlformats.org/spreadsheetml/2006/main"
    sst = ('<?xml version="1.0"?><sst xmlns="%s">%s</sst>' % (
        ns, "".join('<si><t xml:space="preserve">%s</t></si>' % t for t in shared)))
...
def test_no_network_code_in_the_builder():
    ...
    for banned in ("import requests", "urllib.request", "urlopen", "httpx", "socket."):
        assert banned not in src, f"builder must not perform network I/O (found {banned!r})"
